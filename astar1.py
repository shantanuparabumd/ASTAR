...
#         print(xg,yg,)
    ...
    def draw_vector(self,current,neighbor):
        ...
        start_point = (current[4][1],current[4][0])
        end_point = (neighbor[4][1],neighbor[4][0])
...
#         print(current,distance)
    ...
    def astar(self):
        ...
        start=(0,start_cost_to_goal,0,0,self.START,0)
...
#                     print(neighbor)
                    if not neighbor==None:
                        ...
                        if not self.check_dup(neighbor[4],close_list) and not neighbor[0]==-1:
                            if not self.check_dup(neighbor[4],open_list.queue) or neighbor[0]==float('inf'):
...
#                                 print("Neigbor",neighbor)
                                ...
                                if neighbor[5]>current[5]+cost_of_action:
                                    neighbor[2]=current[3]
                                    neighbor[0]=neighbor[5]+self.cost_to_goal(neighbor[4][0],neighbor[4][1])
        tracker.append(current)          
        end_time = time.time()
